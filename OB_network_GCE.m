function [Mitral GraProximal GraDistal param InputCurrent] = OB_network_GCE_clean(inputFile)
...
for ii = 1:length(Cin)
    LHS(:,ii) = Restgradist + (Cin(ii)^2)./(1e-4 .* param.rhoCa .* wVDCCGradist .* mbarrest);
end
...
%     
    m(:,tt) = m(:,tt - 1) + (param.dt ./ taum(:)).*(mbar(:) - m(:,tt - 1));
...
%     
    ...
    Vgradist(:,tt) = Vgradist(:,tt - 1) + (param.dt ./ taugradist(:)) .* ...
        ((ImitgradistAMPA(:) + ImitgradistNMDA(:) + Iproxdist(:))...
...
Wext = 0.001*(sort(rand(1,param.nMitral),'descend'))+0.0134; % hard-coded weights matrix
